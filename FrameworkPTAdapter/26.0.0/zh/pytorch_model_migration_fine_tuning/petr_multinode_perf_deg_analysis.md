...
2.  根因解析
    -   **1节点到5节点性能明显劣化的原因（从上至下，重要性递减）**<br>

        a.  总差距为693ms，算子耗时差距310ms。
    ...
    -   **1节点性能问题的原因（从上至下，重要性递减）**<br>
        a.  总差距为390ms，算子耗时差距占了240ms。<br>
        b.  模型原本就存在严重的快慢卡问题，syncbn的大量同步导致了大量同步等待时延。
    ...
    -   **快慢卡原因分析**<br>
        a.  差距从Python CPU侧开始，随着API的调用，差距慢慢拉大，造成影响的API为aten。<br>
        b.  Python侧存在部分函数耗时异常，如图2所示，与之前某些用户存在的现象类似，疑似CPU资源被抢占。
        ...
        **图 2** 慢卡profiling示意图
        ...
        c.  不同卡的batchmatmulv2耗时差距巨大，从170ms到240ms波动。
